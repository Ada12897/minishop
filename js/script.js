...
for (let i = 0; i < products.length; i++) {
  ...
  if (product.category === "vinyl") {
    console.log("Винил:", product.title, "-", product.price.toLocaleString(), "₸");
  } else if (product.category === "players") {
    console.log("Проигрыватель:", product.title, "-", product.price.toLocaleString(), "₸");
  } else {
    console.log("Инструмент/аксессуар:", product.title, "-", product.price.toLocaleString(), "₸");
  }
}
...
if (payBtn) {
  payBtn.addEventListener("click", () => {
    ...
    if (!nameInput.value || cardInput.value.length !== 16 || expInput.value.length !== 5 || cvcInput.value.length !== 3) {
